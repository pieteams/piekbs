...
PieKBS 是面向 Agent 的本地优先知识搜索引擎。它把原始文档蒸馏为结构化、可 review 的 Markdown 知识库，通过两个 MCP 工具——`kb_search` 和 `kb_page`——让 Agent 按自己的节奏搜索和深读。

![PieKBS Screenshot](image-001.png)

## 设计理念

PieKBS 基于一个核心观察：**不要你以为，我要我以为**——Agent 更愿意使用外部知识工具的方式和人用搜索引擎一样，用不同关键词多次查询，顺着关联链接展开，最终自己综合结论。它们不需要系统打包好的答案，需要的是能自主验证、自主汇总的原始材料。

因此 PieKBS 的职责不是回答问题，而是确保 Agent 搜索时能找到正确的文档，并能完整读取。
...
Agent 应主动迭代搜索、跟随 `related` 链接、交叉验证、自行得出结论。PieKBS 不生成答案。

## PieKBS vs RAG

传统 RAG 检索上下文后交给 LLM 来回答，然后返回给Agent。PieKBS 把原始材料交给 Agent，让 Agent 自己推理。

```text
RAG:       用户提问 → 检索上下文 → LLM 生成答案
PieKBS:  Agent 搜索 → Agent 深读 → Agent 自己综合
```
...
PieKBS wiki bundle 遵循 [OKF v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf) 规范。
...
把任意 Markdown 文件放入 `raw/`，`piekbs serve` 的 watcher 会自动完成蒸馏 + 建索引。LLM 把原始资料提取为结构化 source-note，写入 `wiki/source-notes/`，包含：
...
> **macOS Intel（x86_64）：** 暂无预编译包。GitHub Actions 的 Intel macOS runner 已于 2025 年 4 月停用。Intel Mac 用户请在本机从源码构建：`CGO_ENABLED=1 go build -tags fts5 -o piekbs ./cmd/piekbs/`
...
**Windows：** 解压 zip，运行 `piekbs.exe serve`（或 `piekbs.exe stdio` 用于 MCP）。将目录加入 `PATH`。无需 CGO，纯 Go 二进制。

**鸿蒙 PC（社区实验性支持）：** PieKBS 暂无鸿蒙 PC 官方发行包。但由于核心二进制无需 CGO（纯 Go + SQLite），可通过社区 [Harmonybrew](https://harmonybrew.dev) 包管理器在鸿蒙 PC 上原生构建。环境搭建方法参考 [ohos_go_cgo](https://github.com/ohos-go/ohos_go_cgo)。
...
`piekbs init` 会把内置的撰写规范和页面模板复制到 KB 的 `schema/` 目录：
...
piekbs init           # 初始化 KB 目录并复制 schema/模板
piekbs serve          # 启动服务：MCP + Web UI + 文件监控
piekbs index          # 构建/更新 FTS 索引
piekbs status         # 索引统计
piekbs lint           # 健康检查 wiki 页面
```

## 命令参考

所有命令都接受全局 `--kb <path>` 参数（默认取 `$WIKILOOP_KB`，再退回 `~/piekbs-kb`）。

| 命令 | 说明 |
|---|---|
| `piekbs init [--force]` | 初始化 KB 目录并复制内置 schema/模板。 |
| `piekbs serve` | 启动常驻服务：HTTP MCP（`/mcp`）+ Web UI + 文件监控。无子命令时的默认行为。 |
| `piekbs index` | 从 `wiki/` 和 `raw/` 的 markdown 构建/更新 FTS 索引。 |
| `piekbs search <query>` | FTS 关键词搜索，输出带路径和摘要的排序结果。 |
| `piekbs synthesize [--topic X] [--full]` | 从 source-notes 生成 concept/comparison/decision 页面。 |
| `piekbs synthesize --gaps --topic X` | 对某主题做知识缺口分析。 |
| `piekbs import-lark <URL>` | 导入飞书/Lark Wiki 页面及内嵌多维表格到 `raw/lark/`。需要已登录的 `lark-cli`。 |
| `piekbs lint` | 健康检查 wiki 页面：缺失 frontmatter 字段、断裂的来源链接。 |
...
推荐使用 HTTP 方式：一个 PieKBS 进程，所有 Agent 共用——Claude Code、Cursor、VS Code（Copilot）、Windsurf、Trae、Codex、Hermes、OpenClaw 等均可接入。
...
> macOS 也可直接双击 PieKBS.app 启动（menubar 图标）。
...
托管环境中，将 PieKBS 安装在持久卷上，通过 **stdio** 调用——PieKBS 作为 Agent 宿主的子进程启动，watcher 在后台自动运行。
...
`piekbs serve` 启动后内置 watcher 会自动监控 KB 目录变化、触发蒸馏和建索引，无需额外配置。

如果需要让 PieKBS **开机自启、后台常驻**，可以安装为系统服务（macOS launchd / Linux systemd）：
