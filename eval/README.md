...
用 RAGAS 四项指标对比 PieKBS 与 Naive RAG 的检索质量。
...
**结论**：PieKBS 在答案相关性（+23.5%）和上下文召回（+27.5%）上显著优于 Naive RAG。
...
**前置条件**：
- PieKBS serve 已启动（`piekbs serve`）
- `~/.hermes/piekbs-kb/config.yaml` 中已配置 LLM（distill 段）
