...
PieKBS 是面向 Agent 的本機優先知識搜尋引擎。它把原始文件蒸餾為結構化、可 review 的 Markdown 知識庫，透過兩個 MCP 工具——`kb_search` 和 `kb_page`——讓 Agent 依自己的節奏搜尋和深讀。

![PieKBS Screenshot](image-001.png)

## 設計理念

PieKBS 基於一個核心觀察：**Agent 使用外部知識工具的方式和人用搜尋引擎一樣**——用不同關鍵詞多次查詢，順著關聯連結展開，最終自己綜合結論。它們不需要系統打包好的答案，需要的是能自主驗證、自主彙總的原始材料。

因此 PieKBS 的職責不是回答問題，而是確保 Agent 搜尋時能找到正確的文件，並能完整讀取。
...
Agent 應主動迭代搜尋、跟隨 `related` 連結、交叉驗證、自行得出結論。PieKBS 不生成答案。

## PieKBS vs RAG

傳統 RAG 檢索上下文後交給 LLM 來回答，然後返回給 Agent。PieKBS 把原始材料交給 Agent，讓 Agent 自己推理。

```text
RAG:       用戶提問 → 檢索上下文 → LLM 生成答案
PieKBS:  Agent 搜尋 → Agent 深讀 → Agent 自己綜合
```
...
PieKBS wiki bundle 遵循 [OKF v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf) 規範。
...
把任意 Markdown 檔案放入 `raw/`，`piekbs serve` 的 watcher 會自動完成蒸餾 + 建索引。LLM 把原始資料提取為結構化 source-note，寫入 `wiki/source-notes/`，包含：
...
> **macOS Intel（x86_64）：** 暫無預編譯套件。GitHub Actions 的 Intel macOS runner 已於 2025 年 4 月停用。Intel Mac 用戶請在本機從原始碼建置：`CGO_ENABLED=1 go build -tags fts5 -o piekbs ./cmd/piekbs/`

**macOS：** 開啟 DMG，將 PieKBS 拖入 Applications。App 以 menubar 圖示形式運行。

> **Gatekeeper 警告：** PieKBS 未使用 Apple 開發者憑證簽章。首次啟動時，macOS 可能會阻止應用程式執行。解決方法：右鍵點擊 PieKBS.app → 選擇「打開」→ 在彈出對話框中點擊「打開」。或在終端機中執行 `xattr -cr /Applications/PieKBS.app`。
...
**Windows：** 解壓 zip，執行 `piekbs.exe serve`（或 `piekbs.exe stdio` 用於 MCP）。將目錄加入 `PATH`。無需 CGO，純 Go 二進位。

**鴻蒙 PC（社群實驗性支援）：** PieKBS 暫無鴻蒙 PC 官方發行套件。但由於核心二進位無需 CGO（純 Go + SQLite），可透過社群 [Harmonybrew](https://harmonybrew.dev) 套件管理器在鴻蒙 PC 上原生建置。環境搭建方法參考 [ohos_go_cgo](https://github.com/ohos-go/ohos_go_cgo)。
...
`piekbs init` 會把內建的撰寫規範和頁面範本複製到 KB 的 `schema/` 目錄：
...
piekbs init           # 初始化 KB 目錄並複製 schema/範本
piekbs serve          # 啟動服務：MCP + Web UI + 檔案監控
piekbs index          # 建置/更新 FTS 索引
piekbs status         # 索引統計
piekbs lint           # 健康檢查 wiki 頁面
```

## 命令參考

所有命令都接受全域 `--kb <path>` 參數（預設取 `$PIEKBS_KB`，再退回 `~/piekbs-kb`）。

| 命令 | 說明 |
|---|---|
| `piekbs init [--force]` | 初始化 KB 目錄並複製內建 schema/範本。 |
| `piekbs serve` | 啟動常駐服務：HTTP MCP（`/mcp`）+ Web UI + 檔案監控。無子命令時的預設行為。 |
| `piekbs index` | 從 `wiki/` 和 `raw/` 的 markdown 建置/更新 FTS 索引。 |
| `piekbs search <query>` | FTS 關鍵詞搜尋，輸出帶路徑和摘要的排序結果。 |
| `piekbs synthesize [--topic X] [--full]` | 從 source-notes 生成 concept/comparison/decision 頁面。 |
| `piekbs synthesize --gaps --topic X` | 對某主題做知識缺口分析。 |
| `piekbs import-lark <URL>` | 匯入飛書/Lark Wiki 頁面及內嵌多維表格到 `raw/lark/`。需要已登入的 `lark-cli`。 |
| `piekbs lint` | 健康檢查 wiki 頁面：缺失 frontmatter 欄位、斷裂的來源連結。 |
| `piekbs status` | 列印索引統計（文件數、索引大小）。 |
| `piekbs service <install\|uninstall\|start\|stop\|status\|logs>` | 管理系統服務（launchd / systemd）。 |

**LLM 設定**（KB 根目錄的 `config.yaml` 的 `distill` 段）是 `distill` 和 `synthesize` 的必要條件。

## MCP Server

PieKBS 透過 MCP 協議對外暴露 KB 工具。
...
推薦使用 HTTP 方式：一個 PieKBS 行程，所有 Agent 共用——Claude Code、Cursor、VS Code（Copilot）、Windsurf、Trae、Codex、Hermes、OpenClaw 等均可接入。
...
> macOS 也可直接雙擊 PieKBS.app 啟動（menubar 圖示）。
...
使用 `Authorization: Bearer`（推薦）或 `x-api-key`（已棄用）進行認證。金鑰對應 `config.yaml` 中的 `server.api_key`。未設定 api_key 時可省略 `headers`。

---

### 場景二：託管 Agent 環境（Hermes / OpenClaw 等）

託管環境中，將 PieKBS 安裝在持久卷上，透過 **stdio** 呼叫——PieKBS 作為 Agent 宿主的子行程啟動，watcher 在背景自動運行。
...
PieKBS 內建純 Go 轉換引擎，支援 PDF、Word、Excel、PPT、EPUB、HTML、CSV 和郵件檔案。無需額外安裝，二進位檔案轉換開箱即用。
...
| 內容類型 | 寫入路徑 |
|---|---|
| 文章、筆記、參考資料（Markdown/文字） | `$PIEKBS_KB/raw/<你的分類>/<slug>.md` |
| Agent 轉換的 PDF / Word / Excel / EPUB 內容 | `$PIEKBS_KB/raw/converted/<slug>.md` |

`raw/converted/` 中的檔案被視為已轉換，直接進入蒸餾，跳過轉換步驟。`raw/` 下其他路徑均經過完整流水線處理（轉換 → 建索引 → 蒸餾）。

`raw/` 下的子目錄組織方式不限——PieKBS 不強制規定固定結構。

## 系統服務（可選）

`piekbs serve` 啟動後內建 watcher 會自動監控 KB 目錄變化、觸發蒸餾和建索引，無需額外設定。

如果需要讓 PieKBS **開機自啟、背景常駐**，可以安裝為系統服務（macOS launchd / Linux systemd）：
...
日誌：`{PIEKBS_KB}/index/watcher.log`
