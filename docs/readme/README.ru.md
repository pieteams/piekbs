...
Все команды принимают глобальный флаг `--kb <path>` (по умолчанию `$PIEKBS_KB`, затем `~/piekbs-kb`).
...
**2. Внешние инструменты не требуются.**

PieKBS включает встроенную конвертацию на чистом Go для файлов PDF, Word, Excel, PPT, EPUB, HTML, CSV и Email. Дополнительная установка не нужна — конвертация бинарных файлов работает сразу.
...
|---|---|
| Статьи, заметки, справочные материалы (Markdown/текст) | `$PIEKBS_KB/raw/<ваша-категория>/<slug>.md` |
| Контент PDF/Word/Excel/EPUB, конвертированный агентом | `$PIEKBS_KB/raw/converted/<slug>.md` |

Файлы в `raw/converted/` рассматриваются как уже конвертированные и отправляются прямо на дистилляцию, минуя шаг конвертации. Все остальные пути под `raw/` обрабатываются через полный конвейер (конвертация → индексирование → дистилляция).
...
Журналы: `{PIEKBS_KB}/index/watcher.log`
