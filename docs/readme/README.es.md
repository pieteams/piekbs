...
> **Advertencia de Gatekeeper:** PieKBS no está firmado con un certificado de desarrollador de Apple. En el primer inicio, macOS puede bloquear la app. Solución: clic derecho en PieKBS.app → seleccionar "Abrir" → hacer clic en "Abrir" en el diálogo. O ejecutar en Terminal: `xattr -cr /Applications/PieKBS.app`
...
PieKBS admite dos métodos de autenticación:
...
2. **x-api-key** (obsoleto, se eliminará en 6 meses)
```http
POST /mcp HTTP/1.1
Host: localhost:8080
...
Use `Authorization: Bearer` (recomendado) o `x-api-key` (obsoleto) para autenticación. La clave corresponde a `server.api_key` en `config.yaml`. Omita `headers` si no se establece api_key.
