...
PieKBS는 에이전트를 위한 로컬 우선 지식 검색 엔진입니다. 원본 문서를 구조화되고 검토 가능한 Markdown 지식 베이스로 정제한 다음, 두 가지 MCP 도구 — `kb_search`와 `kb_page` — 를 통해 에이전트가 자신의 속도에 맞춰 검색하고 깊이 읽을 수 있게 합니다.

![PieKBS Screenshot](image-001.png)

## 설계 철학

PieKBS는 하나의 관찰에서 출발합니다: **에이전트는 외부 지식 도구를 인간이 검색 엔진을 사용하는 것처럼 활용합니다** — 다양한 각도에서 여러 쿼리를 날리고, 링크를 따라가며, 스스로 결론을 종합합니다. 미리 포장된 답을 원하는 것이 아니라, 스스로 결론을 도출할 수 있는 원자재가 필요합니다.

따라서 PieKBS의 역할은 질문에 답하는 것이 아닙니다. 에이전트가 무언가를 검색할 때 올바른 문서를 찾고 — 전체 내용을 읽을 수 있도록 보장하는 것입니다.
...
## 에이전트가 PieKBS를 사용하는 방법

에이전트는 세 가지 MCP 도구를 통해 PieKBS와 상호작용합니다:
...
에이전트는 반복적으로 검색하고, `related` 링크를 따라가며, 출처를 교차 검증하고, 스스로 결론을 내릴 것으로 예상됩니다. PieKBS는 답변을 생성하지 않습니다.

## PieKBS vs RAG

전통적인 RAG는 컨텍스트를 검색하여 LLM에게 답변하도록 전달합니다. PieKBS는 에이전트에게 원자재를 전달하고 에이전트 스스로 추론하게 합니다.

```text
RAG:       사용자 질문 → 컨텍스트 검색 → LLM 답변
PieKBS:  에이전트 검색 → 에이전트 읽기 → 에이전트 합성
```
...
PieKBS 번들은 [OKF v0.1](https://github.com/GoogleCloudPlatform/knowledge-catalog/tree/main/okf)을 준수합니다.
...
`raw/`에 Markdown 파일을 넣습니다. `piekbs serve` watcher가 자동으로 정제 + 인덱싱을 실행합니다. LLM은 `wiki/source-notes/`에 구조화된 source-note를 추출합니다. 포함 내용:
...
> **macOS Intel (x86_64):** 사전 빌드된 릴리스 없음. GitHub Actions가 2025년 4월에 Intel macOS 러너를 중단했습니다. Intel Mac에서 소스 코드로 빌드하세요: `CGO_ENABLED=1 go build -tags fts5 -o piekbs ./cmd/piekbs/`
...
**Windows:** zip을 압축 해제하고 `piekbs.exe serve`를 실행합니다 (MCP용 `piekbs.exe stdio`). 디렉토리를 `PATH`에 추가합니다. CGO 불필요 — 순수 Go 바이너리.

**HarmonyOS PC (커뮤니티, 실험적):** PieKBS는 HarmonyOS PC용으로 공식 출시되지 않습니다. 그러나 핵심 바이너리는 CGO가 필요 없으므로 (순수 Go + SQLite), 커뮤니티 패키지 관리자 [Harmonybrew](https://harmonybrew.dev)를 사용하여 HarmonyOS PC에서 네이티브로 빌드할 수 있습니다. HarmonyOS PC에서 Go + CGO 설정 가이드는 [ohos_go_cgo](https://github.com/ohos-go/ohos_go_cgo)를 참조하세요.
...
`piekbs init`은 번들된 저작 규칙과 페이지 템플릿으로 KB의 `schema/` 디렉토리를 채웁니다:
...
piekbs init           # KB 디렉토리 스캐폴딩 및 schema/템플릿 복사
piekbs serve          # 서버 시작: MCP + Web UI + 파일 watcher
piekbs index          # FTS 인덱스 빌드/업데이트
piekbs status         # 인덱스 통계
piekbs lint           # wiki 페이지 상태 점검
```

## 명령어 참조

모든 명령은 전역 `--kb <path>` 플래그를 허용합니다 (기본값은 `$PIEKBS_KB`, 그 다음 `~/piekbs-kb`).

| 명령 | 설명 |
|---|---|
| `piekbs init [--force]` | KB 디렉토리 스캐폴딩 및 번들된 schema/템플릿 복사. |
| `piekbs serve` | 장시간 실행 서버 시작: HTTP MCP (`/mcp`) + Web UI + 파일 watcher. 서브커맨드 없을 때 기본값. |
| `piekbs index` | `wiki/`와 `raw/` markdown에서 FTS 인덱스 빌드/업데이트. |
| `piekbs search <query>` | FTS 키워드 검색; 경로와 스니펫이 있는 순위별 결과 출력. |
| `piekbs synthesize [--topic X] [--full]` | source-notes에서 concept/comparison/decision 페이지 생성. |
| `piekbs synthesize --gaps --topic X` | 주제에 대한 지식 격차 분석. |
| `piekbs import-lark <URL>` | Lark/Feishu Wiki 페이지와 임베디드 테이블을 `raw/lark/`로 가져옵니다. 로그인된 `lark-cli` 필요. |
| `piekbs lint` | wiki 페이지 상태 점검: 누락된 frontmatter 필드, 깨진 소스 링크. |
| `piekbs status` | 인덱스 통계 출력 (문서 수, 인덱스 크기). |
| `piekbs service <install\|uninstall\|start\|stop\|status\|logs>` | OS 서비스 관리 (launchd / systemd). |

**LLM 설정** (KB 루트 아래 `config.yaml`의 `distill` 섹션)은 `distill`과 `synthesize`에 필요합니다.

## MCP 서버

PieKBS는 MCP 프로토콜을 통해 KB 도구를 노출합니다.
...
HTTP 모드 권장: 하나의 PieKBS 프로세스를 모든 에이전트가 공유 — Claude Code, Cursor, VS Code (Copilot), Windsurf, Trae, Codex, Hermes, OpenClaw 등.
...
> macOS에서: PieKBS.app을 더블 클릭하여 메뉴바 아이콘으로 실행합니다.
...
호스팅 환경 (Hermes, OpenClaw 등)에서는 PieKBS를 영구 볼륨에 설치하고 **stdio**를 통해 호출합니다 — PieKBS는 에이전트 호스트의 하위 프로세스로 시작하며, watcher는 자동으로 백그라운드에서 실행됩니다.
...
`piekbs serve`에는 KB 디렉토리를 자동으로 모니터링하고, 정제를 트리거하고, 인덱스를 재구성하는 내장 watcher가 포함됩니다. 추가 설정이 필요하지 않습니다.

PieKBS가 **부팅 시 시작하고 백그라운드에서 실행**되게 하려면 시스템 서비스로 설치합니다 (macOS launchd / Linux systemd):
...
로그: `{PIEKBS_KB}/index/watcher.log`
